...
@MetaInfServices
@Namespaces({
    @Namespace(uri = "urn:infinispan:config:store:jpa-eclipselink:7.2", root = "jpa-eclipselink-store"),
    @Namespace(root = "jpa-eclipselink-store")
})
public class JpaEclipselinkConfigurationParser72 implements ConfigurationParser {
    @Override
    public void readElement(XMLExtendedStreamReader reader,
        ...
        switch (element) {
        case JPA_ECLIPSELINK_STORE: {
            parseJpaCacheStore(reader, builder.persistence().addStore(JpaEclipselinkStoreConfigurationBuilder.class));
            break;
        }
        default: {
    ...
    private void parseJpaCacheStore(XMLExtendedStreamReader reader, JpaEclipselinkStoreConfigurationBuilder builder)
